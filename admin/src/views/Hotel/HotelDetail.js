...
import { getHotelDetail, postHotelDetail } from '../../assets/api/hotel';
...
function HotelDetail(props) {
  ...
  const uploadSuccessList = (type, data) => {
    // console.log(111, detail.pics, type, data);s
  ...
  const onFinish = (values) => {
    ...
    let newParams = _.omit(params, ['create_time', 'update_time']);

    // return;
    postHotelDetail(newParams).then(() => {
  ...
  return (
    <div>
      <Content>
        <Card loading={loading} title="酒店详情" style={{ marginTop: '10px' }}>
          {detail.title && (
            <Form
              form={form}
              labelCol={{ span: 4 }}
